...
export default function RestaurantPage({ params }: PageProps) {
  ...
  // Sync cart items with the latest availability from the menu
  useEffect(() => {
    if (restaurant?.menu_items && items.length > 0) {
      items.forEach(item => {
        const latest = restaurant.menu_items.find(m => m.id === item.id);
        if (latest && latest.is_available !== item.is_available) {
          // Update the item in the cart with the latest availability
          updateQuantity(item.id, 0, latest);
        }
      });
    }
  }, [restaurant, items.length]); // Only run when restaurant data or cart size changes

  const handleAddToCart = (item: MenuItem) => {
    if (!user || role !== 'customer') {
      toast.error("Please login as a customer to add items to your bag");
      return;
    }
    addItem({
      id: item.id,
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc]">
      <Card className="max-w-md w-full p-8 text-center rounded-[2.5rem] border-none shadow-2xl">
        ...
        <Link href="/restaurants">
          <Button className="bg-orange-600 hover:bg-orange-700 rounded-2xl h-14 px-8 font-black cursor-pointer">
            Back to Restaurants
          </Button>
        </Link>
      </Card>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-20">
      ...
      <div className="max-w-7xl mx-auto px-8 -mt-16 relative z-20">
        <div className={`grid grid-cols-1 ${showCart ? 'lg:grid-cols-4' : 'lg:grid-cols-1'} gap-8`}>
          {/* Main Menu Section */}
          <div className={showCart ? 'lg:col-span-3' : 'lg:col-span-1'}>
            <div className="space-y-8">
              ...
              <div className="space-y-12">
                {(restaurant.categories && restaurant.categories.length > 0) ? (
                  <Tabs defaultValue={restaurant.categories?.[0]?.name} className="w-full">
                    ...
                    {restaurant.categories?.map((category) => (
                      <TabsContent key={category.id} value={category.name} className="space-y-6">
                        <div className="grid md:grid-cols-2 gap-6">
                                  ...
                                  <div className="p-6 flex flex-col justify-between h-full space-y-4">
                                    ...
                                    <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                                      ...
                                      <Button
                                        ...
                                        className="bg-orange-600 hover:bg-orange-700 rounded-xl h-12 px-6 font-bold shadow-lg shadow-orange-100 transition-all active:scale-95 cursor-pointer"
                                      >
                                        {showCart ? 'Add to Bag' : 'Order Now'}
                                      </Button>
                                    </div>
                                  </div>
                                </CardContent>
                          ...
                          {(restaurant.menu_items || [])
                            .filter(m => m.category_id === category.id)
                            .filter(m => m.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <h2 className="text-2xl font-black text-slate-900">All Items</h2>
                      <Badge variant="outline" className="rounded-lg">{filteredItemsCount} items</Badge>
                    </div>
                    <div className="grid md:grid-cols-2 gap-6">
                      {(restaurant.menu_items || [])
                        .filter(m => m.name.toLowerCase().includes(searchQuery.toLowerCase()))
                      ...
                      {filteredItemsCount === 0 && (
                        <div className="col-span-full text-center py-20 bg-white rounded-[3rem] shadow-sm border-2 border-dashed border-slate-100">
                          <p className="text-slate-400 font-bold">No menu items found for this restaurant.</p>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {/* Checkout / Bag Section - Only for customers */}
          {showCart && (
                  ...
                  {items.length === 0 ? (
                    ...
                    <>
                        ...
                        {items.map((item) => (
                          <div key={item.id} className="flex items-center justify-between group">
                            ...
                            <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-xl">
                              <button
                                onClick={() => {
                                  const latestItem = restaurant.menu_items?.find(m => m.id === item.id);
                                  if (latestItem?.is_available === false) {
                                    toast.error("This item is currently sold out");
                                    return;
                                  }
                                  updateQuantity(item.id, -1, latestItem);
                                }}
                                disabled={restaurant.menu_items?.find(m => m.id === item.id)?.is_available === false}
                                className="w-6 h-6 rounded-lg bg-white flex items-center justify-center font-black text-slate-400 hover:text-orange-600 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                              >
                                -
                              </button>
                              <span className="font-black text-slate-900 text-sm min-w-[20px] text-center">{item.quantity}</span>
                              <button
                                onClick={() => {
                                  const latestItem = restaurant.menu_items?.find(m => m.id === item.id);
                                  if (latestItem?.is_available === false) {
                      ...
                      <div className="border-t border-slate-100 pt-6 space-y-4">
                        ...
                        <Link href="/cart">
                          <Button className="w-full bg-orange-600 hover:bg-orange-700 h-16 rounded-[1.5rem] font-black text-lg shadow-xl shadow-orange-100 transition-all active:scale-95 group cursor-pointer">
