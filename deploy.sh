...
print(f"Syncing {len(env_vars)} env vars to Vercel...")
...
if "created" in result:
    print(f"✅ {len(result['created'])} env vars synced to Vercel!")
else:
    print(result)
PYEOF
    ;;
  ...
  deploy)
    ...
    echo ""
    echo "✅ Done! Live at: https://yourlocaleats.vercel.app"
    ;;

  all)
    echo "🔄 Syncing env vars + deploying..."
    echo ""
    "$0" sync && "$0" deploy
    ;;
  ...
  *)
    ...
    echo "  sync    - Sync .env vars to Vercel"
