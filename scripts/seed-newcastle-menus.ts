/**
 * seed-newcastle-menus.ts
 * Seeds REAL scraped menu data from scraper/newcastle_menus.json
 * into the database for all Newcastle restaurants.
 *
 * Run: npx tsx scripts/seed-newcastle-menus.ts
 */
...
// Realistic high-quality Unsplash image URLs for different food types
...
function pickImage(restaurantName: string, itemName: string, category: string): string {
  ...
  if (nameLower.includes("chicken") || nameLower.includes("wing") || nameLower.includes("strip") || nameLower.includes("nugget")) return IMAGES.friedChicken;
  if (nameLower.includes("burger")) return IMAGES.burger;
  ...
  if (nameLower.includes("bacon") || nameLower.includes("sausage") || nameLower.includes("egg")) return IMAGES.breakfast;
  if (nameLower.includes("coke") || nameLower.includes("fanta") || nameLower.includes("sprite") || nameLower.includes("water") || nameLower.includes("juice")) return IMAGES.soda;
...
async function main() {
  ...
  const imagesPath = path.join(__dirname, "../scraper/newcastle_images.json");
  const scrapedImages: Record<string, string> = fs.existsSync(imagesPath)
    ? JSON.parse(fs.readFileSync(imagesPath, "utf-8"))
    : {};

  // Load scraped data
  const jsonPath = path.join(__dirname, "../scraper/newcastle_menus.json");
  if (!fs.existsSync(jsonPath)) {
    console.error("❌ scraper/newcastle_menus.json not found. Run scraper first.");
    process.exit(1);
  }
  ...
  console.log("Fetching Newcastle restaurants from DB...");
  const dbRestaurants = await db
    .select()
    .from(restaurants)
    .where(eq(restaurants.location, "Newcastle"));

  console.log(`Found ${dbRestaurants.length} restaurant(s) in Newcastle.\n`);
  ...
  for (const [, scraped_r] of Object.entries(scraped)) {
    ...
    const normalizeStr = (s: string) => s.toLowerCase().replace(/[’‘]/g, "'").trim();
    ...
    // Check if already seeded (if we want to clear first, let's delete existing menu items)
    ...
    if (existing.length > 0) {
      console.log(`   Clearing ${existing.length} existing mock menu items for "${dbMatch.name}"...`);
