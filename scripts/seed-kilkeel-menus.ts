/**
 * seed-kilkeel-menus.ts
 * Seeds REAL scraped menu data from scraper/kilkeel_menus.json
 * into the database for all Kilkeel restaurants.
 * Clears any existing mock/dummy menu items first.
 *
 * Run: npx tsx scripts/seed-kilkeel-menus.ts
 */
...
function pickImage(restaurantName: string, itemName: string, category: string): string {
  ...
  // Off-license specific image selection
  if (restLower.includes("kwm wines") || restLower.includes("kwm wine")) {
    if (nameLower.includes("wine") || nameLower.includes("prosecco") || nameLower.includes("champagne") || nameLower.includes("shiraz") || nameLower.includes("sauvignon") || nameLower.includes("merlot") || nameLower.includes("pinot")) return IMAGES.wine;
    ...
    return IMAGES.spirit;
  }

  // General specific overrides first
  ...
  if (catLower === "drinks") {
    if (nameLower.includes("wine") || nameLower.includes("prosecco") || nameLower.includes("champagne") || nameLower.includes("rose")) return IMAGES.wine;
    if (nameLower.includes("spirit") || nameLower.includes("vodka") || nameLower.includes("gin") || nameLower.includes("whiskey") || nameLower.includes("rum") || nameLower.includes("brandy")) return IMAGES.spirit;
...
function cleanItems(
  ...
  for (const catBlock of categories) {
    ...
    for (const item of catBlock.items) {
      ...
      // Truncate name to 148 chars (DB column is varchar(150))
      const name = rawName.length > 148 ? rawName.substring(0, 145) + "..." : rawName;
      ...
      if (finalCat === "Menu" || finalCat === "General") {
        finalCat = categorizeMenuItem(item.name, item.description || "", restaurantName);
      }
...
async function main() {
  ...
  const imagesPath = path.join(__dirname, "../scraper/kilkeel_images.json");
  const scrapedImages: Record<string, string> = fs.existsSync(imagesPath)
    ? JSON.parse(fs.readFileSync(imagesPath, "utf-8"))
    : {};

  const jsonPath = path.join(__dirname, "../scraper/kilkeel_menus.json");
  if (!fs.existsSync(jsonPath)) {
    console.error("❌ scraper/kilkeel_menus.json not found. Run scraper first.");
    process.exit(1);
  }
  ...
  console.log("Fetching Kilkeel restaurants from DB...");
  const dbRestaurants = await db
    .select()
    .from(restaurants)
    .where(eq(restaurants.location, "Kilkeel"));

  console.log(`Found ${dbRestaurants.length} restaurant(s) in Kilkeel.\n`);
  ...
  for (const [, scraped_r] of Object.entries(scraped)) {
    ...
    const normalizeStr = (s: string) => s.toLowerCase().replace(/['']/g, "'").replace(/[&]/g, "and").trim();

    const dbMatch = dbRestaurants.find(dbr => {
      const dbN = normalizeStr(dbr.name);
      const scrN = normalizeStr(scraped_r.name);
    ...
    if (!dbMatch) {
      console.log(`⚠  No DB match for "${scraped_r.name}" — skipping.`);
      totalSkipped++;
      continue;
    }
    ...
    console.log(`→  Seeding "${dbMatch.name}" — ${items.length} real scraped items...`);
